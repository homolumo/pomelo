...
Service.prototype.beforeFilter = function(msg, session, cb) {
  ...
  var next = function(err, resp, opts) {
    ...
    if (fun.constructor.name === 'AsyncFunction') {
      fun(msg, session).then(r => next()).catch(e => next(e));
    } else {
  ...
  function next(err) {
    ...
    if (fun.constructor.name === 'AsyncFunction') {
      if (err && handler.error) {
        handler.error(err).then(r => next(err));
      } else {
        if (handler === fun)
          fun(msg, session, resp).then(r => next()).catch(e => next(e));
        else
          handler.after(msg, session, resp).then(r => next()).catch(e => next(e));
      }
    } else {
      if (handler === fun)
        fun(err, msg, session, resp, next);
      else
