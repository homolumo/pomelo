...
Service.prototype.handle = function(routeRecord, msg, session, cb) {
  ...
  if (!Array.isArray(msg)) {
    promiss = handler[method](msg, session);
  } else {
    msg.push(session);
    // msg.push(callback);
    promiss = handler[method].apply(handler, msg);
  }
  promiss.then(r => callback(null, r)).catch((e) => {
    logger.error('route [%j] emit error: %j', msg.__route__, e);
    callback(e, null);
  });
